...
function(target_enable_debug_options TARGET_NAME)
    set(oneValueArgs ENABLE_EDIT_AND_CONTINUE ENABLE_DEBUG_INFO
                     DEBUG_INFO_LEVEL)
    ...
    if(NOT TARGET_NAME OR NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_enable_debug_options() called without valid TARGET")
    endif()

    if(NOT DEFINED ARG_DEBUG_INFO_LEVEL)
        set(ARG_DEBUG_INFO_LEVEL 2
        )# Default to level 2 (good balance of info vs size)
    endif()
    message(
        STATUS
            "Configuring debug options for target '${TARGET_NAME}' (${CURRENT_COMPILER})"
    )

    get_current_compiler(CURRENT_COMPILER)
    if("${CURRENT_COMPILER}" MATCHES "MSVC|CLANG-MSVC")
        _configure_msvc_debug_options(
            ${TARGET_NAME} PRIVATE ${ARG_ENABLE_EDIT_AND_CONTINUE}
            ${ARG_ENABLE_DEBUG_INFO})
    elseif("${CURRENT_COMPILER}" MATCHES "CLANG.*|GCC")
        _configure_gcc_clang_debug_options(
            ${TARGET_NAME} PRIVATE ${ARG_ENABLE_DEBUG_INFO}
            ${ARG_DEBUG_INFO_LEVEL})
    else()
        message(
            STATUS
                "Debug options not configured for compiler: ${CURRENT_COMPILER}"
        )
    endif()
endfunction()
...
function(enable_global_debug_options)
    ...
    get_property(already_applied GLOBAL
                 PROPERTY PROJECT_GLOBAL_DEBUG_OPTIONS_ENABLED)
    if(already_applied)
    ...
    elseif("${CURRENT_COMPILER}" MATCHES "CLANG.*|GCC")
        ...
        message(
            STATUS
                "Global debug options not configured for compiler: ${CURRENT_COMPILER}"
        )
    endif()
...
function(_configure_msvc_debug_options TARGET_NAME SCOPE_NAME
    ...
    if(ENABLE_EDIT_AND_CONTINUE)
        ...
        message(
            STATUS
                "  - Incremental linking: enabled (required for Edit and Continue)"
        )
        ...
        get_target_property(EXISTING_COMPILE_OPTIONS ${TARGET_NAME}
                            COMPILE_OPTIONS)
        if(EXISTING_COMPILE_OPTIONS)
            list(FILTER EXISTING_COMPILE_OPTIONS EXCLUDE REGEX "/guard:cf")
            set_target_properties(
                ${TARGET_NAME} PROPERTIES COMPILE_OPTIONS
                                          "${EXISTING_COMPILE_OPTIONS}")
            message(
                STATUS
                    "  - Control Flow Guard disabled (incompatible with Edit and Continue)"
            )
        endif()
...
function(_configure_gcc_clang_debug_options TARGET_NAME SCOPE_NAME
         ENABLE_DEBUG_INFO DEBUG_INFO_LEVEL)
    set(DEBUG_OPTIONS "")
...
function(_configure_global_msvc_debug_options)
    if(${ENABLE_EDIT_AND_CONTINUE})
        ...
        append_flags_if_missing(CMAKE_CXX_FLAGS_DEBUG "/ZI"
                                "Global CXX Debug flags with Edit and Continue")
        append_flags_if_missing(CMAKE_C_FLAGS_DEBUG "/ZI"
                                "Global C Debug flags with Edit and Continue")

        # Enable incremental linking for debug builds
        append_flags_if_missing(
            CMAKE_EXE_LINKER_FLAGS_DEBUG "/INCREMENTAL"
            "Global EXE linker Debug flags for Edit and Continue")
        append_flags_if_missing(
            CMAKE_SHARED_LINKER_FLAGS_DEBUG "/INCREMENTAL"
            "Global SHARED linker Debug flags for Edit and Continue")
    ...
    elseif(${ENABLE_DEBUG_INFO})
        ...
        append_flags_if_missing(CMAKE_CXX_FLAGS_DEBUG "/Zi"
                                "Global CXX Debug flags with debug info")
        append_flags_if_missing(CMAKE_C_FLAGS_DEBUG "/Zi"
                                "Global C Debug flags with debug info")
...
function(_configure_global_gcc_clang_debug_options)
    if(${ENABLE_DEBUG_INFO})
        ...
        if(DEBUG_FLAG)
            append_flags_if_missing(CMAKE_CXX_FLAGS_DEBUG "${DEBUG_FLAG}"
                                    "Global CXX Debug flags with debug info")
            append_flags_if_missing(CMAKE_C_FLAGS_DEBUG "${DEBUG_FLAG}"
                                    "Global C Debug flags with debug info")
        endif()
    endif()
endfunction()
