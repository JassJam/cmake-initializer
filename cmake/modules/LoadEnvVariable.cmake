...
function(target_load_env_variable TARGET_NAME VARIABLE VALUE)
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_load_env_file: Target '${TARGET_NAME}' does not exist")
        return()
    endif()
    ...
    if(VALUE STREQUAL "")
        if(DEFINED ENV{${VARIABLE}})
            ...
            message(
                FATAL_ERROR
                    "Environment variable '${VARIABLE}' is not defined in the system environment and no value was provided."
            )
        endif()
    endif()
...
function(target_load_env_file TARGET_NAME FILENAME)
    ...
    foreach(j RANGE 0 ${ENV_LENGTH} 1)
        ...
        target_compile_definitions(
            ${TARGET_NAME} PRIVATE "${ENV_KEY_FINAL}=${ENV_VALUE_FINAL}")
    endforeach()
endfunction()
...
function(target_load_env_files TARGET_NAME)
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_load_env_files: Target '${TARGET_NAME}' does not exist")
        return()
    endif()
...
function(target_load_env_files_with_fallback TARGET_NAME BASE_PATH)
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_load_env_files_with_fallback: Target '${TARGET_NAME}' does not exist"
        )
        return()
    endif()
    ...
    set(CONFIG_ENV_FILE
        "${BASE_DIR}/${BASE_NAME_CLEAN}.env.${BUILD_TYPE_LOWER}")

    set(ENV_FILES_TO_LOAD "${DEFAULT_ENV_FILE}" "${DEFAULT_ENV_FILE}.local"
                          "${CONFIG_ENV_FILE}" "${CONFIG_ENV_FILE}.local")
...
function(load_env_file FILE_PATH OUT_KEYS OUT_VALUES)
    if(EXISTS "${FILE_PATH}")
        ...
        foreach(line IN LISTS ENV_LINES)
            string(REGEX MATCH "^([A-Za-z_][A-Za-z0-9_]*)=(.*)$" _match
                         "${line}")
            if(_match)
                string(REGEX REPLACE "^([A-Za-z_][A-Za-z0-9_]*)=(.*)$" "\\1"
                                     ENV_KEY "${line}")
                string(REGEX REPLACE "^([A-Za-z_][A-Za-z0-9_]*)=(.*)$" "\\2"
                                     ENV_VAL "${line}")
