...
function(target_enable_hardening TARGET_NAME SCOPE_NAME)
    ...
    elseif(NOT ${SCOPE_NAME} IN_LIST CMAKE_TARGET_SCOPE_TYPES)
        message(
            FATAL_ERROR
                "Invalid SCOPE_NAME '${SCOPE_NAME}' for target_enable_hardening()"
        )
    endif()

    _should_enable_ubsan_minimal_runtime(ENABLE_UBSAN_MINIMAL_RUNTIME)
    _get_hardening_options(NEW_COMPILE_OPTIONS NEW_LINK_OPTIONS
                           NEW_CXX_DEFINITIONS)
    ...
    if(NOT "${NEW_COMPILE_OPTIONS}" STREQUAL "")
        target_compile_options(${TARGET_NAME} ${SCOPE_NAME}
                               ${NEW_COMPILE_OPTIONS})
    endif()
    ...
    if(NOT "${NEW_CXX_DEFINITIONS}" STREQUAL "")
        target_compile_definitions(${TARGET_NAME} ${SCOPE_NAME}
                                                  ${NEW_CXX_DEFINITIONS})
    endif()
endfunction()
...
function(enable_global_hardening)
    ...
    message(
        STATUS "** Enable global hardening to all targets and all dependencies")
    ...
    if(NOT "${NEW_COMPILE_OPTIONS}" STREQUAL "")
        ...
        append_flags_if_missing(CMAKE_CXX_FLAGS "${COMPILE_FLAGS_STR}"
                                "Global CXX flags with hardening")
        append_flags_if_missing(CMAKE_C_FLAGS "${COMPILE_FLAGS_STR}"
                                "Global C flags with hardening")
    endif()

    if(NOT "${NEW_LINK_OPTIONS}" STREQUAL "")
        string(JOIN " " LINK_FLAGS_STR ${NEW_LINK_OPTIONS})
        append_flags_if_missing(CMAKE_EXE_LINKER_FLAGS "${LINK_FLAGS_STR}"
                                "Global EXE linker flags with hardening")
        append_flags_if_missing(CMAKE_SHARED_LINKER_FLAGS "${LINK_FLAGS_STR}"
                                "Global SHARED linker flags with hardening")
    endif()
    ...
    set(PROJECT_GLOBAL_HARDENING_ENABLED_CACHE
        TRUE
        CACHE INTERNAL "Guards enable_global_hardening() across reconfigures")
...
function(_configure_msvc_hardening COMPILE_OPTIONS_VAR LINK_OPTIONS_VAR
         DEFINITIONS_VAR)
    if(ENABLE_EDIT_AND_CONTINUE)
        message(
            STATUS
                "*** Hardening MSVC flags: /DYNAMICBASE /NXCOMPAT /CETCOMPAT (Control Flow Guard disabled due to Edit and Continue)"
        )
    else()
        message(
            STATUS
                "*** Hardening MSVC flags: /DYNAMICBASE /guard:cf /NXCOMPAT /CETCOMPAT"
        )
    ...
    set(${COMPILE_OPTIONS_VAR}
        ${${COMPILE_OPTIONS_VAR}}
        PARENT_SCOPE)
    set(${LINK_OPTIONS_VAR}
        ${${LINK_OPTIONS_VAR}}
        PARENT_SCOPE)
endfunction()

# Helper function to configure GCC/Clang hardening flags
function(_configure_gcc_clang_hardening COMPILE_OPTIONS_VAR LINK_OPTIONS_VAR
         DEFINITIONS_VAR CURRENT_COMPILER)
    message(STATUS "*** GLIBC++ Assertions (vector[], string[], ...) enabled")
    list(APPEND ${DEFINITIONS_VAR} _GLIBCXX_DEBUG _GLIBCXX_DEBUG_PEDANTIC
         _GLIBCXX_ASSERTIONS)

    if(NOT CMAKE_BUILD_TYPE MATCHES "Debug")
        message(STATUS "*** g++/clang _FORTIFY_SOURCE=3 enabled")
        list(APPEND ${COMPILE_OPTIONS_VAR} -U_FORTIFY_SOURCE
             -D_FORTIFY_SOURCE=3)
    endif()
    ...
    if(SHOULD_ENABLE_MINIMAL_RUNTIME)
        ...
        if(MINIMAL_RUNTIME)
            list(APPEND ${COMPILE_OPTIONS_VAR} -fsanitize=undefined
                 -fsanitize-minimal-runtime -fno-sanitize-recover=undefined)
            list(APPEND ${LINK_OPTIONS_VAR} -fsanitize=undefined
                 -fsanitize-minimal-runtime -fno-sanitize-recover=undefined)
            message(STATUS "*** ubsan minimal runtime enabled")
        else()
            message(
                STATUS "*** ubsan minimal runtime NOT enabled (not supported)")
        endif()
    else()
        message(
            STATUS
                "*** ubsan minimal runtime NOT enabled (incompatible with other sanitizers)"
        )
    endif()
...
function(_get_hardening_options COMPILE_OPTIONS_VAR LINK_OPTIONS_VAR
         DEFINITIONS_VAR)
    ...
    if("${CURRENT_COMPILER}" MATCHES "MSVC")
        _configure_msvc_hardening(NEW_COMPILE_OPTIONS NEW_LINK_OPTIONS
                                  NEW_CXX_DEFINITIONS)
    elseif("${CURRENT_COMPILER}" MATCHES "CLANG|GCC|EMSCRIPTEN")
        _configure_gcc_clang_hardening(
            NEW_COMPILE_OPTIONS NEW_LINK_OPTIONS NEW_CXX_DEFINITIONS
            "${CURRENT_COMPILER}")
    else()
