...
function(target_enable_static_analysis TARGET_NAME)
    set(options ENABLE_CLANG_TIDY ENABLE_CPPCHECK ENABLE_EXCEPTIONS)
    ...
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_enable_static_analysis: Target '${TARGET_NAME}' does not exist"
        )
    endif()
...
function(_configure_clang_tidy TARGET_NAME ENABLE_EXCEPTIONS)
    ...
    _add_clang_tidy_custom_target(${TARGET_NAME} ${ENABLE_EXCEPTIONS}
                                  ${CLANG_TIDY_EXE})
endfunction()

# Helper function to create clang-tidy custom targets
function(_add_clang_tidy_custom_target TARGET_NAME ENABLE_EXCEPTIONS
         CLANG_TIDY_EXE)
    if(NOT CLANG_TIDY_EXE)
        return()
    endif()
    ...
    list(APPEND CXX_CLANG_TIDY_ARGS
         "--config-file=${PROJECT_SOURCE_DIR}/.clang-tidy")
    file(RELATIVE_PATH BINARY_DIR_RELATIVE "${PROJECT_SOURCE_DIR}"
         "${CMAKE_BINARY_DIR}")

    if(WIN32)
        list(APPEND CXX_CLANG_TIDY_ARGS
             "--extra-arg=-Wno-dll-attribute-on-redeclaration")
        list(APPEND CXX_CLANG_TIDY_ARGS
             "--extra-arg=-Wno-inconsistent-dllimport")
    endif()
    ...
    elseif(CURRENT_COMPILER MATCHES "CLANG.*|GCC|EMSCRIPTEN")
        ...
        if(CURRENT_COMPILER MATCHES "GCC")
            ...
            list(APPEND CXX_CLANG_TIDY_ARGS
                 "--extra-arg=-Wno-duplicated-branches")
    ...
    if(MSVC)
        set_property(TARGET ${TARGET_NAME}
                     PROPERTY VS_GLOBAL_EnableMicrosoftCodeAnalysis false)
        set_property(TARGET ${TARGET_NAME}
                     PROPERTY VS_GLOBAL_EnableClangTidyCodeAnalysis true)
        set_property(TARGET ${TARGET_NAME} PROPERTY VS_GLOBAL_RunCodeAnalysis
                                                    true)
    endif()
endfunction()
...
function(lint_source_file SOURCE_FILE)
    ...
    set(oneValueArgs ENABLE_CLANG_TIDY ENABLE_CPPCHECK ENABLE_EXCEPTIONS)
    ...
    if(NOT IS_ABSOLUTE "${SOURCE_FILE}")
        get_filename_component(SOURCE_FILE "${SOURCE_FILE}" ABSOLUTE BASE_DIR
    ...
    if(NOT EXISTS "${SOURCE_FILE}")
        message(
            FATAL_ERROR
                "lint_source_file: Source file '${SOURCE_FILE}' does not exist")
    endif()
    ...
    if(ARG_ENABLE_CLANG_TIDY)
        ...
        _build_clang_tidy_command(CLANG_TIDY_COMMAND "${CLANG_TIDY_EXE}"
                                  "${SOURCE_FILE}" ${ARG_ENABLE_EXCEPTIONS})
...
function(_find_clang_tidy OUT_CLANG_TIDY)
    ...
    if(NOT CLANG_TIDY_EXE AND WIN32)
        ...
        if(VSWHERE_EXE)
            ...
            execute_process(
                COMMAND
                    "${VSWHERE_EXE}" -latest -products * -requires
                    Microsoft.VisualStudio.Component.VC.Llvm.Clang -property
                    installationPath
            ...
            if(VS_INSTALLATION_PATH)
                ...
                if(CLANG_TIDY_EXE)
                    message(
                        STATUS
                            "** clang-tidy found via vswhere: ${CLANG_TIDY_EXE}"
                    )
                else()
                    ...
                    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
                        message(
                            STATUS
                                "** clang-tidy not found, but MSVC compiler detected"
                        )
                        message(
                            STATUS
                                "** Consider using MSVC's built-in /analyze flag or installing LLVM tools"
                        )
                        # Set a flag to indicate MSVC static analysis could be used instead
                        set(MSVC_STATIC_ANALYSIS_AVAILABLE
                            TRUE
                            PARENT_SCOPE)
                    endif()
                endif()
...
function(_build_clang_tidy_command OUT_CLANG_TIDY_COMMAND CLANG_TIDY_EXE
         SOURCE_FILE ENABLE_EXCEPTIONS)
