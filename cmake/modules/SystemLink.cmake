...
function(target_include_system_directories TARGET_NAME)
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_include_system_directories() called with invalid TARGET_NAME: ${TARGET_NAME}"
        )
    endif()
    ...
    foreach(scope IN ITEMS CMAKE_TARGET_SCOPE_TYPES)
        foreach(lib_include_dirs IN LISTS ARG_${scope})
            if(${scope} STREQUAL "INTERFACE" OR ${scope} STREQUAL "PUBLIC")
                ...
                target_include_directories(${TARGET_NAME} SYSTEM ${scope}
                                           ${lib_include_dirs})
            endif()
        endforeach()
    endforeach()
...
function(target_include_system_library TARGET_NAME SCOPE_NAME LIB_NAME)
    ...
    if(NOT ${SCOPE_NAME} IN_LIST CMAKE_TARGET_SCOPE_TYPES)
        message(
            FATAL_ERROR
                "target_add_compiler_warnings() called with invalid SCOPE: ${SCOPE_NAME}"
        )
    endif()

    # check if this is a target
    if(TARGET ${lib})
        get_target_property(lib_include_dirs ${LIB_NAME}
                            INTERFACE_INCLUDE_DIRECTORIES)
        if(lib_include_dirs)
            target_include_system_directories(${TARGET_NAME} ${SCOPE_NAME}
                                              ${lib_include_dirs})
        else()
            message(
                TRACE
                "${LIB_NAME} library does not have the INTERFACE_INCLUDE_DIRECTORIES property."
            )
        endif()
    endif()
endfunction()
