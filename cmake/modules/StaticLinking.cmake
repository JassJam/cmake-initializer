...
function(target_enable_static_linking TARGET_NAME SCOPE_NAME)
    ...
    if(DEFINED STATIC_LINKING_ENABLED AND STATIC_LINKING_ENABLED)
        message(
            STATUS "Static linking already enabled for target: ${TARGET_NAME}")
        return()
    endif()
    ...
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_enable_static_linking: TARGET argument is required")
    endif()
    ...
    elseif(NOT ${SCOPE_NAME} IN_LIST CMAKE_TARGET_SCOPE_TYPES)
        message(
            FATAL_ERROR
                "target_enable_static_linking: Invalid scope '${SCOPE_NAME}' specified. Must be one of: ${CMAKE_TARGET_SCOPE_TYPES}."
        )
    endif()
    ...
    if(CURRENT_COMPILER MATCHES "CLANG.*|GCC")
        ...
        message(
            STATUS
                "Enabling static runtime linking for ${CURRENT_COMPILER} target: ${TARGET_NAME}"
        )
    elseif(CURRENT_COMPILER STREQUAL "MSVC")
        ...
        message(
            STATUS
                "Enabling static runtime linking for MSVC target: ${TARGET_NAME}"
        )
    elseif(CURRENT_COMPILER STREQUAL "INTEL")
        target_link_options(${TARGET_NAME} ${SCOPE_NAME} "-static-intel")
        message(
            STATUS
                "Enabling static runtime linking for Intel target: ${TARGET_NAME}"
        )
    elseif(CURRENT_COMPILER STREQUAL "EMSCRIPTEN")
        ...
        target_link_options(${TARGET_NAME} ${SCOPE_NAME}
                            "SHELL:-s STANDALONE_WASM=1")
        target_link_options(${TARGET_NAME} ${SCOPE_NAME} "SHELL:-s WASM=1")
        message(
            STATUS
                "Enabling static runtime linking for Emscripten target: ${TARGET_NAME}"
        )
    else()
        message(
            WARNING
                "Static runtime linking not supported for compiler: ${CURRENT_COMPILER}"
        )
    endif()
endfunction()
...
function(enable_static_linking)
    ...
    message(
        STATUS
            "Enabling static linking of runtime libraries for ${CURRENT_COMPILER}"
    )
    ...
    elseif(CURRENT_COMPILER STREQUAL "EMSCRIPTEN")
        ...
        message(
            WARNING
                "Static linking not configured for compiler: ${CURRENT_COMPILER}"
        )
    endif()

    set(STATIC_LINKING_ENABLED
        TRUE
        PARENT_SCOPE)
