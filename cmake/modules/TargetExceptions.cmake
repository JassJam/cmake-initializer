...
function(target_configure_exceptions TARGET_NAME ON_OFF)
    if(GLOBAL_EXCEPTIONS_SET)
        message(
            TRACE
            "Global exceptions are already enabled, ignoring target-specific settings"
        )
        return()
    endif()
...
function(_configure_exceptions OUT_COMPILE_FLAGS OUT_LINK_FLAGS ON_OFF)
    ...
    if(CURRENT_COMPILER STREQUAL "MSVC" OR CURRENT_COMPILER STREQUAL
                                           "CLANG-MSVC")
        if(ON_OFF)
            set(${OUT_COMPILE_FLAGS}
                "/EHsc"
                PARENT_SCOPE)
        else()
            set(${OUT_COMPILE_FLAGS}
    ...
    elseif(CURRENT_COMPILER MATCHES "CLANG.*|GCC|EMSCRIPTEN")
        if(ON_OFF)
            ...
            if(CURRENT_COMPILER MATCHES "EMSCRIPTEN")
                set(${OUT_LINK_FLAGS}
                    "-fexceptions" "SHELL:-s DISABLE_EXCEPTION_CATCHING=0"
                    PARENT_SCOPE)
            endif()
        else()
            set(${OUT_COMPILE_FLAGS}
        ...
        message(
            WARNING
                "Unknown compiler ${CURRENT_COMPILER}, not applying exceptions settings"
        )
    endif()
endfunction()
