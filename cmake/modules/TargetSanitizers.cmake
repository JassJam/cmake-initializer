...
function(enable_global_sanitizers)
    ...
    if(CMAKE_BUILD_TYPE STREQUAL "Release")
        message(
            TRACE
            "Disabling global sanitizers in Release mode (use Debug or RelWithDebInfo for sanitizers)"
        )
        return()
    endif()
    ...
    if(ENABLE_UBSAN)
        if(CURRENT_COMPILER STREQUAL "MSVC")
            message(
                WARNING "Undefined behavior sanitizer is not supported on MSVC")
        else()
    ...
    if(CURRENT_COMPILER MATCHES "MSVC")
        ...
        if(CURRENT_COMPILER MATCHES "CLANG")
            message(
                WARNING
                    "Clang with MSVC target does not support sanitizers, disabling"
            )
        ...
        elseif(MSVC_VERSION GREATER_EQUAL 1928) # VS 2019 16.9+
            ...
            foreach(sanitizer ${LIST_OF_SANITIZERS})
                append_flags_if_missing(
                    CMAKE_CXX_FLAGS "/fsanitize=${sanitizer}"
                    "Global CXX flags with sanitizers")
                append_flags_if_missing(CMAKE_C_FLAGS "/fsanitize=${sanitizer}"
                                        "Global C flags with sanitizers")
            endforeach()
            ...
            if("address" IN_LIST LIST_OF_SANITIZERS)
                message(
                    STATUS
                        "** AddressSanitizer detected - forcing release runtime (MultiThreaded instead of MultiThreadedDebug)"
                )
                ...
                set(CMAKE_MSVC_RUNTIME_LIBRARY
                    ...
                    CACHE
                        STRING
                        "MSVC runtime library (overridden for AddressSanitizer compatibility)"
                        FORCE)
            endif()
            ...
            append_flags_if_missing(
                ...
                "Global linker flags for executables with sanitizers")
            append_flags_if_missing(
                CMAKE_SHARED_LINKER_FLAGS "/INCREMENTAL:NO"
                "Global linker flags for shared libraries with sanitizers")
            append_flags_if_missing(
                CMAKE_MODULE_LINKER_FLAGS "/INCREMENTAL:NO"
                "Global linker flags for modules with sanitizers")
            ...
            if("address" IN_LIST LIST_OF_SANITIZERS)
                ...
                if(VSWHERE_EXECUTABLE)
                    ...
                    execute_process(
                        COMMAND "${VSWHERE_EXECUTABLE}" -latest -property
                                installationPath
                ...
                if(ASAN_RUNTIME_DLL AND EXISTS "${ASAN_RUNTIME_DLL}")
                    message(
                        STATUS
                            "** Found AddressSanitizer runtime DLL at: ${ASAN_RUNTIME_DLL}"
                    )
                    # Set a global property so we can copy the DLL for each executable
                    set_property(GLOBAL PROPERTY ASAN_RUNTIME_DLL_PATH
                                                 "${ASAN_RUNTIME_DLL}")
                else()
                    message(
                        WARNING
                            "AddressSanitizer runtime DLL (${ASAN_DLL_PATTERN}) not found. You may need to add the Visual Studio bin directory to your PATH."
                    )
                    ...
                    if(VSWHERE_EXECUTABLE)
                        ...
                        message(
                            STATUS "** vswhere not found, used fallback search")
                    endif()
                endif()
            endif()

            # Always add debug info when using sanitizers to avoid C5072 warning
            append_flags_if_missing(CMAKE_CXX_FLAGS "/Zi"
                                    "Global CXX flags with debug info")
            append_flags_if_missing(CMAKE_C_FLAGS "/Zi"
                                    "Global C flags with debug info")

            # Disable the ASAN warning about missing debug info since we're adding it
            append_flags_if_missing(CMAKE_CXX_FLAGS "/wd5072"
                                    "Global CXX flags to disable ASAN warning")
            append_flags_if_missing(CMAKE_C_FLAGS "/wd5072"
                                    "Global C flags to disable ASAN warning")
        else()
            message(
                WARNING
                    "AddressSanitizer requires Visual Studio 2019 16.9 or later. Current MSVC version: ${MSVC_VERSION}"
            )
            message(
                STATUS
                    "Disabling global sanitizers due to unsupported MSVC version"
            )
            return()
        endif()
    ...
    elseif(CURRENT_COMPILER MATCHES "CLANG.*|GCC|EMSCRIPTEN")
        ...
        append_flags_if_missing(CMAKE_CXX_FLAGS "${SANITIZER_FLAGS}"
                                "Global CXX flags with sanitizers")
        append_flags_if_missing(CMAKE_C_FLAGS "${SANITIZER_FLAGS}"
                                "Global C flags with sanitizers")
        append_flags_if_missing(CMAKE_EXE_LINKER_FLAGS "${SANITIZER_FLAGS}"
                                "Global EXE linker flags with sanitizers")
        append_flags_if_missing(CMAKE_SHARED_LINKER_FLAGS "${SANITIZER_FLAGS}"
                                "Global SHARED linker flags with sanitizers")
    endif()
endfunction()
