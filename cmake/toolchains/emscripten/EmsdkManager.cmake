...
function(ensure_emsdk_available)
    ...
    if(DEFINED ENV{EMSDK} AND EXISTS "$ENV{EMSDK}")
        ...
        find_program(
            EMCC_TEST emcc
            PATHS "$ENV{EMSDK}/upstream/emscripten"
            NO_DEFAULT_PATH)
        if(EMCC_TEST)
            ...
            message(
                STATUS
                    "EMSDK found at $ENV{EMSDK} but compilers not accessible. Will install locally."
            )
        endif()
    endif()
    ...
    if(ENABLE_EMSDK_AUTO_INSTALL)
        message(
            STATUS
                "EMSDK not found. Automatically installing it locally to ${LOCAL_EMSDK_DIR}"
        )
    else()
        message(
            FATAL_ERROR
                "EMSDK not found. Please install it manually or enable ENABLE_EMSDK_AUTO_INSTALL to download it automatically."
        )
    endif()
    ...
    if(NOT GIT_FOUND)
        message(
            FATAL_ERROR
                "Git is required to download EMSDK. Please install Git first.")
    endif()
    ...
    execute_process(
        COMMAND
            ...
            https://github.com/emscripten-core/emsdk.git "${LOCAL_EMSDK_DIR}"
...
function(get_emsdk_toolchain_file output_var)
    ...
    if(DEFINED ENV{EMSDK})
        ...
        set(TOOLCHAIN_FILE
            "${PROJECT_SOURCE_DIR}/.emsdk/upstream/emscripten/cmake/Modules/Platform/Emscripten.cmake"
        )
    endif()

    if(NOT EXISTS "${TOOLCHAIN_FILE}")
        message(
            FATAL_ERROR "Emscripten toolchain file not found: ${TOOLCHAIN_FILE}"
        )
    endif()
...
function(verify_and_setup_emscripten_compilers)
    ...
    set(CMAKE_TOOLCHAIN_FILE
        "${TOOLCHAIN_FILE}"
        CACHE FILEPATH "Emscripten toolchain file" FORCE)
    ...
    if(NOT CMAKE_C_COMPILER STREQUAL EMCC_PATH)
        set(CMAKE_C_COMPILER
            "${EMCC_PATH}"
            CACHE FILEPATH "Emscripten C compiler" FORCE)
    endif()
...
function(_activate_local_emsdk emsdk_dir)
    ...
    if(CMAKE_HOST_WIN32)
        ...
        if(NODE_PATH
           ...
           AND JAVA_PATH)
            set(ENV{PATH}
                "${EMSCRIPTEN_ROOT};${NODE_PATH};${PYTHON_PATH};${JAVA_PATH};$ENV{PATH}"
            )
        elseif(NODE_PATH AND PYTHON_PATH)
            set(ENV{PATH}
                "${EMSCRIPTEN_ROOT};${NODE_PATH};${PYTHON_PATH};$ENV{PATH}")
        else()
