...
function(target_register_asset TARGET_NAME)
    ...
    if(NOT TARGET ${TARGET_NAME})
        message(
            FATAL_ERROR
                "target_register_asset: Target '${TARGET_NAME}' does not exist")
    endif()
    ...
    if(IS_ABSOLUTE "${ARG_FILE}")
        ...
        if(ARG_URL)
            set(ASSET_SOURCE_PATH
                "${CMAKE_CURRENT_SOURCE_DIR}/_assets/${ARG_FILE}")
        else()
    ...
    string(MAKE_C_IDENTIFIER "${TARGET_NAME}_asset_${ASSET_DEST_RELATIVE}"
                             ASSET_TARGET_NAME)
    ...
    if(ARG_URL)
        if(NOT EXISTS "${ASSET_SOURCE_PATH}")
            message(
                STATUS
                    "Asset '${ARG_FILE}' not found, will download from: ${ARG_URL}"
            )
        ...
        elseif(ARG_HASH)
            ...
            if(HASH_MATCH)
                ...
                if(NOT "${EXISTING_HASH}" STREQUAL "${EXPECTED_HASH}")
                    message(
                        STATUS
                            "Asset '${ARG_FILE}' hash mismatch (expected: ${EXPECTED_HASH}, got: ${EXISTING_HASH}), will re-download"
                    )
                    set(NEED_DOWNLOAD TRUE)
                else()
                    message(
                        STATUS
                            "Asset '${ARG_FILE}' hash verified, no download needed"
                    )
                endif()
            else()
                message(
                    WARNING
                        "Invalid hash format for asset '${ARG_FILE}': ${ARG_HASH}"
                )
            endif()
        endif()
        ...
        if(NEED_DOWNLOAD)
            ...
            if(ARG_HASH)
                file(
                    DOWNLOAD "${ARG_URL}" "${ASSET_SOURCE_PATH}"
            ...
            if(DOWNLOAD_ERROR)
                ...
                if(ARG_REQUIRED)
                    message(
                        FATAL_ERROR
                            "Failed to download asset '${ARG_FILE}': ${DOWNLOAD_ERROR_MSG}\nLog: ${DOWNLOAD_LOG}"
                    )
                else()
                    message(
                        WARNING
    ...
    if(NOT EXISTS "${ASSET_SOURCE_PATH}")
        if(ARG_REQUIRED)
            message(
                FATAL_ERROR
                    "Required asset '${ARG_FILE}' not found at: ${ASSET_SOURCE_PATH}"
            )
        else()
            message(
                WARNING "Asset '${ARG_FILE}' not found at: ${ASSET_SOURCE_PATH}"
            )
            return()
        endif()
    endif()
    ...
    add_custom_target(
        ${ASSET_TARGET_NAME}
        COMMAND ${CMAKE_COMMAND} -E copy_if_different "${ASSET_SOURCE_PATH}"
    ...
    get_filename_component(
        DEST_DIR "$<TARGET_FILE_DIR:${TARGET_NAME}>/${ASSET_DEST_RELATIVE}"
        DIRECTORY)
    add_custom_command(
    ...
    message(
        STATUS
            "Registered asset for target '${TARGET_NAME}': ${ARG_FILE} -> ${ASSET_DEST_RELATIVE}"
    )
endfunction()
...
function(target_register_assets TARGET_NAME)
    ...
    foreach(FILE ${ARG_ASSETS})
        ...
        target_register_asset(${TARGET_NAME} FILE "${FILE}" DESTINATION
                              "${DESTINATION}")
    endforeach()
endfunction()
