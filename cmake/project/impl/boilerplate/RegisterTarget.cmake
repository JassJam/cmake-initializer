...
function(_register_target_common target)
    ...
    if(DEFINED ARG_CXX_STANDARD)
        set_target_properties(
            ${target}
            PROPERTIES CXX_STANDARD ${ARG_CXX_STANDARD}
    ...
    if(ARG_INCLUDE_DIRS)
        ...
        foreach(_inc IN LISTS ARG_INCLUDE_DIRS)
            if(_inc STREQUAL "PUBLIC"
               OR _inc STREQUAL "PRIVATE"
               OR _inc STREQUAL "INTERFACE")
                set(_vis "${_inc}")
            else()
                ...
                if(_vis STREQUAL "PRIVATE")
                    target_include_directories(
                        ${target} PRIVATE "$<BUILD_INTERFACE:${_inc}>")
                elseif(_vis STREQUAL "INTERFACE")
                    target_include_directories(
                        ${target} INTERFACE "$<BUILD_INTERFACE:${_inc}>"
                                            "$<INSTALL_INTERFACE:include>")
                else()
                    target_include_directories(
                        ${target} PUBLIC "$<BUILD_INTERFACE:${_inc}>"
                                         "$<INSTALL_INTERFACE:include>")
                endif()
            endif()
        endforeach()
...
function(register_header_only_library name)
    set(oneValArgs NAMESPACE EXPORT_SET INSTALL_DESTINATION CXX_STANDARD
                   HEADER_BASE_DIR)
    set(multValArgs HEADERS INCLUDE_DIRS LINK_LIBS COMPILE_DEFINITIONS
                    PROPERTIES)
    ...
    if(ARG_HEADERS)
        target_sources(
            ${name} INTERFACE FILE_SET HEADERS BASE_DIRS
                              "${ARG_HEADER_BASE_DIR}" FILES ${ARG_HEADERS})
    endif()
    ...
    if(ARG_INCLUDE_DIRS)
        foreach(_inc IN LISTS ARG_INCLUDE_DIRS)
            ...
            target_include_directories(
                ${name} INTERFACE "$<BUILD_INTERFACE:${_inc}>"
                                  "$<INSTALL_INTERFACE:include>")
        endforeach()
    endif()
...
function(register_library name)
    ...
    set(multValArgs
        ...
        COMPILE_DEFINITIONS
        PROPERTIES)
    cmake_parse_arguments(PARSE_ARGV 1 ARG "${optionArgs}" "${oneValArgs}"
                          "${multValArgs}")
    ...
    if(ARG_HEADERS)
        target_sources(
            ${name} PUBLIC FILE_SET HEADERS BASE_DIRS "${ARG_HEADER_BASE_DIR}"
                           FILES ${ARG_HEADERS})
    endif()
    ...
    if(ARG_CXX_MODULES)
        ...
        target_sources(
            ${name}
            PUBLIC FILE_SET CXX_MODULES BASE_DIRS "${CMAKE_CURRENT_SOURCE_DIR}"
    ...
    if(DEFINED ARG_EXPORT_HEADER)
        ...
        generate_export_header(${name} EXPORT_FILE_NAME "${_export_file}"
                               ${_geh_extra})
        ...
        target_include_directories(
            ${name} PUBLIC "$<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}>"
                           "$<INSTALL_INTERFACE:include>")
    ...
    endforeach()
    foreach(_mv INCLUDE_DIRS LINK_LIBS COMPILE_OPTIONS COMPILE_DEFINITIONS
                PROPERTIES)
        if(ARG_${_mv})
...
function(register_executable name)
    ...
    if(ARG_HEADERS)
        target_sources(
            ${name} PRIVATE FILE_SET HEADERS BASE_DIRS "${ARG_HEADER_BASE_DIR}"
                            FILES ${ARG_HEADERS})
    endif()
    ...
    if(ARG_CXX_MODULES)
        ...
        target_sources(
            ${name}
            PRIVATE FILE_SET CXX_MODULES BASE_DIRS
                    "${CMAKE_CURRENT_SOURCE_DIR}" FILES ${ARG_CXX_MODULES})
    endif()
    ...
    set(_forward CXX_STANDARD ${ARG_CXX_STANDARD} INSTALL_DESTINATION
...
function(register_test name)
    ...
    if(ARG_HEADERS)
        target_sources(
            ${name} PRIVATE FILE_SET HEADERS BASE_DIRS
                            "${CMAKE_CURRENT_SOURCE_DIR}" FILES ${ARG_HEADERS})
    endif()
...
function(register_emscripten name)
    if(NOT DEFINED EMSCRIPTEN)
        message(
            STATUS
                "[register_emscripten] Skipping '${name}' — not an Emscripten build"
        )
        return()
    endif()
    set(optionArgs
    ...
    if(ARG_HTML_TEMPLATE)
        ...
        _register_emscripten_write_shell("${_shell}" "${ARG_HTML_TITLE}"
                                         "${ARG_CANVAS_ID}")
    endif()
    ...
    if(DEFINED ARG_INSTALL_DESTINATION)
        install(
            FILES "$<TARGET_FILE_DIR:${name}>/${name}.html"
