...
if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten" OR CMAKE_CROSSCOMPILING)
    message(
        STATUS
            "Cross-compilation detected - XMake will be built with host compiler"
    )
endif()
...
set(XREPO_PLATFORM
    ""
    CACHE STRING "Xrepo package platform")
set(XREPO_ARCH
    ""
    CACHE STRING "Xrepo package architecture")
set(XREPO_TOOLCHAIN
    ""
    CACHE STRING "Xrepo package toolchain")
set(XREPO_XMAKEFILE
    ""
    CACHE STRING "Xmake script file of Xrepo package")
...
function(_install_xmake_program)
    ...
    message(
        STATUS
            "xmake not found, Install it to ${XMAKE_BINARY_DIR} automatically!")
    ...
    if(WIN32)
        set(XMAKE_ARCHIVE_FILE
            ${CMAKE_BINARY_DIR}/xmake-${XMAKE_VERSION}.win32.zip)
        set(XMAKE_ARCHIVE_URL
            https://github.com/xmake-io/xmake/releases/download/v${XMAKE_RELEASE_LATEST}/xmake-${XMAKE_VERSION}.win32.zip
        )
    else()
        set(XMAKE_ARCHIVE_FILE
            ${CMAKE_BINARY_DIR}/xmake-${XMAKE_VERSION}.tar.gz)
        set(XMAKE_ARCHIVE_URL
            https://github.com/xmake-io/xmake/releases/download/v${XMAKE_RELEASE_LATEST}/xmake-${XMAKE_VERSION}.tar.gz
        )
    endif()
    if(NOT EXISTS "${XMAKE_ARCHIVE_FILE}")
        message(STATUS "Downloading xmake from ${XMAKE_ARCHIVE_URL}")
        file(DOWNLOAD "${XMAKE_ARCHIVE_URL}" "${XMAKE_ARCHIVE_FILE}"
             TLS_VERIFY ON)
    endif()
    ...
    if(NOT EXISTS "${XMAKE_BINARY_DIR}")
        ...
        execute_process(
            COMMAND ${CMAKE_COMMAND} -E tar xzf ${XMAKE_ARCHIVE_FILE}
            WORKING_DIRECTORY ${XMAKE_BINARY_DIR}
            RESULT_VARIABLE exit_code)
        if(NOT "${exit_code}" STREQUAL "0")
            message(
                FATAL_ERROR
                    "unzip ${XMAKE_ARCHIVE_FILE} failed, exit code: ${exit_code}"
            )
        endif()
    endif()
    ...
    if(WIN32)
        ...
        message(
            STATUS
                "Configuring xmake with host compiler (not cross-compilation toolchain)"
        )
        ...
        list(APPEND XMAKE_BUILD_ENV --unset=EMSCRIPTEN --unset=EM_CONFIG
             --unset=EM_CACHE --unset=EM_PORTS)
        list(APPEND XMAKE_BUILD_ENV --unset=EMSDK --unset=EMSDK_NODE
             --unset=EMSDK_PYTHON)
        list(APPEND XMAKE_BUILD_ENV --unset=CMAKE_TOOLCHAIN_FILE
             --unset=CMAKE_SYSTEM_NAME)
        list(APPEND XMAKE_BUILD_ENV --unset=CMAKE_CROSSCOMPILING
             --unset=CMAKE_C_COMPILER --unset=CMAKE_CXX_COMPILER)
        ...
        if(HOST_CC)
            ...
            message(
                WARNING
                    "No host C compiler found - this may cause build failures")
        ...
        list(APPEND XMAKE_BUILD_ENV --unset=TARGET_PLATFORM --unset=TARGET_ARCH
             --unset=CROSS_COMPILE)

        # Clean PATH to remove Emscripten tools - get system PATH without EMSDK
        if(DEFINED ENV{PATH})
            string(REPLACE "$ENV{EMSDK}" "" CLEAN_PATH "$ENV{PATH}")
            string(REPLACE "$ENV{EMSDK}/upstream/emscripten" "" CLEAN_PATH
                           "${CLEAN_PATH}")
        ...
        message(
            STATUS
                "Configuring XMake for host platform: ${XMAKE_HOST_PLAT} ${XMAKE_HOST_ARCH}"
        )
        execute_process(
            COMMAND ${XMAKE_BUILD_ENV} ./configure --plat=${XMAKE_HOST_PLAT}
                    --arch=${XMAKE_HOST_ARCH}
            WORKING_DIRECTORY ${XMAKE_SOURCE_DIR}
            RESULT_VARIABLE exit_code)
        if(NOT "${exit_code}" STREQUAL "0")
            message(
                WARNING
                    "Configure xmake failed, exit code: ${exit_code} - XMake will be disabled for this build"
            )
            set(XMAKE_AVAILABLE
                FALSE
                PARENT_SCOPE)
        ...
        execute_process(
            COMMAND ${XMAKE_BUILD_ENV} make -j4
            WORKING_DIRECTORY ${XMAKE_SOURCE_DIR}
            RESULT_VARIABLE exit_code)
        if(NOT "${exit_code}" STREQUAL "0")
            message(
                WARNING
                    "Build xmake failed, exit code: ${exit_code} - XMake will be disabled for this build"
            )
            set(XMAKE_AVAILABLE
                FALSE
                PARENT_SCOPE)
        ...
        execute_process(
            COMMAND ${XMAKE_BUILD_ENV} make install
                    PREFIX=${XMAKE_BINARY_DIR}/install
            WORKING_DIRECTORY ${XMAKE_SOURCE_DIR}
            RESULT_VARIABLE exit_code)
        if(NOT "${exit_code}" STREQUAL "0")
            message(
                WARNING
                    "Install xmake failed, exit code: ${exit_code} - XMake will be disabled for this build"
            )
            set(XMAKE_AVAILABLE
                FALSE
                PARENT_SCOPE)
        ...
        if(EXISTS ${XMAKE_BINARY})
            ...
            message(
                WARNING
                    "XMake binary not found after installation - XMake will be disabled for this build"
            )
            set(XMAKE_AVAILABLE
                FALSE
                PARENT_SCOPE)
...
macro(_detect_xmake_cmd)
    ...
    if(NOT XMAKE_CMD AND XREPO_BOOTSTRAP_XMAKE)
        ...
        if(NOT XMAKE_AVAILABLE)
            message(
                WARNING
                    "XMake build failed - XMake package manager will be disabled for this build"
            )
            return()
        endif()
    endif()
    ...
    if(NOT XMAKE_CMD)
        message(
            WARNING
                "xmake not found and could not be built - XMake package manager disabled. Please install it manually from https://xmake.io"
        )
...
function(_xrepo_detect_json_support)
    ...
    elseif(XREPO_CMD)
        ...
        if(NOT "${exit_code}" STREQUAL "0")
            message(
                FATAL_ERROR
                    "xrepo fetch --help failed, exit code: ${exit_code} -- ${help_output}"
            )
        endif()
        ...
        if(NOT "${help_output}" MATCHES "--json")
            message(
                WARNING
                    "xrepo fetch does not support --json (please upgrade xrepo/xmake to the latest version), "
                    "xrepo_package maybe unreliable to setup package variables")
            set(use_fetch_json OFF)
        endif()
    endif()

    set(XREPO_FETCH_JSON
        ${use_fetch_json}
        CACHE BOOL "Use xrepo JSON output" FORCE)
endfunction()
...
if(NOT XREPO_PACKAGE_DISABLE)
    ...
    if(NOT XMAKE_AVAILABLE)
        message(
            WARNING
                "XMake package manager not available - XMake functions will be disabled"
        )
        return()
    endif()
    ...
    if(CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 3.1)
        message(
            WARNING
                "xrepo: CMAKE_MINIMUM_REQUIRED_VERSION less than 3.1. "
    ...
    if((NOT "${XREPO_TOOLCHAIN}" STREQUAL "") AND ("${XREPO_PLATFORM}" STREQUAL
        ...
        message(
            STATUS
                "xrepo: XREPO_TOOLCHAIN is set but XREPO_PLATFORM is not, this is experimental feature of xmake"
        )
    endif()
endif()
...
function(xrepo_package package)
    ...
    if(DEFINED ARG_CONFIGS)
        if(ARG_CONFIGS MATCHES "\\.lua$")
            ...
            elseif(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${ARG_CONFIGS}")
                set(_config_lua_script
                    "${CMAKE_CURRENT_LIST_DIR}/${ARG_CONFIGS}")
            else()
                message(
                    WARNING
                        "CONFIGS ${ARG_CONFIGS} ends with '.lua' but no file found, taken as normal config"
                )
            endif()
        endif()
        ...
        if(DEFINED _config_lua_script)
            ...
            set_property(
                DIRECTORY
                APPEND
                PROPERTY CMAKE_CONFIGURE_DEPENDS "${_config_lua_script}")
        else()
    ...
    set(_xrepo_cmdargs ${platform} ${arch} ${toolchain} ${includes} ${mode}
                       ${configs})
    ...
    # command again.
    string(REGEX REPLACE ";" " " _xrepo_cmdargs_${package_name}
                         "${XREPO_CMD} install ${_xrepo_cmdargs}")
    if(DEFINED _config_lua_script)
        file(TIMESTAMP ${_config_lua_script}
             _config_lua_script_modify_timestamp)
        set(_xrepo_cmdargs_${package_name}
            "${_xrepo_cmdargs_${package_name}} (mtime: ${_config_lua_script_modify_timestamp})"
        )
    endif()

    if("${_cache_xrepo_cmdargs_${package_name}}" STREQUAL
       "${_xrepo_cmdargs_${package_name}}")
        message(
            STATUS "xrepo: ${package} already installed, using cached variables"
        )
    ...
    execute_process(
        COMMAND
            ...
            ${XREPO_CMD} install --yes ${verbose}
            ${XREPO_BUILD_PARALLEL_JOBS_STR} ${_xrepo_cmdargs}
        RESULT_VARIABLE exit_code)
    if(NOT "${exit_code}" STREQUAL "0")
        message(
            FATAL_ERROR
                "xrepo install ${package} failed, exit code: ${exit_code}")
    endif()
    ...
    set(_cache_xrepo_cmdargs_${package_name}
        "${_xrepo_cmdargs_${package_name}}"
        CACHE INTERNAL "")
endfunction()
...
function(xrepo_target_packages target)
    ...
    foreach(package_name IN LISTS ARG_UNPARSED_ARGUMENTS)
        if(DEFINED ${package_name}_INCLUDE_DIRS)
            message(
                STATUS
                    "xrepo: target_include_directories(${target} SYSTEM ${_visibility} ${${package_name}_INCLUDE_DIRS})"
            )
        ...
        if(DEFINED ${package_name}_LIBRARY_DIRS)
            message(
                STATUS
                    "xrepo: target_link_directories(${target} ${_visibility} ${${package_name}_LIBRARY_DIRS})"
            )
            target_link_directories(${target} ${_visibility}
                                    ${${package_name}_LIBRARY_DIRS})
        endif()
        if((DEFINED ${package_name}_LINK_LIBRARIES) AND (NOT
                                                         ARG_NO_LINK_LIBRARIES))
            message(
                STATUS
                    "xrepo: target_link_libraries(${target} ${_visibility} ${${package_name}_LINK_LIBRARIES})"
            )
            target_link_libraries(${target} ${_visibility}
                                  ${${package_name}_LINK_LIBRARIES})
        endif()
        if((DEFINED ${package_name}_SYS_LIBRARIES) AND (NOT
                                                        ARG_NO_LINK_LIBRARIES))
            message(
                STATUS
                    "xrepo: target_link_libraries(${target} ${_visibility} ${${package_name}_SYS_LIBRARIES})"
            )
            target_link_libraries(${target} ${_visibility}
                                  ${${package_name}_SYS_LIBRARIES})
        endif()
        if(DEFINED ${package_name}_DEFINITIONS)
            message(
                STATUS
                    "xrepo: target_compile_definitions(${target} ${_visibility} ${${package_name}_DEFINITIONS})"
            )
...
macro(_xrepo_set_cmake_prefix_path package_name)
    ...
    foreach(var ${${package_name}_INCLUDE_DIRS})
        ...
        if(NOT "${var}" IN_LIST CMAKE_PREFIX_PATH)
            ...
            message(
                STATUS
                    "xrepo: ${package_name} prepend to CMAKE_PREFIX_PATH: ${_install_dir}"
            )
        endif()
        unset(_install_dir)
    endforeach()
...
function(_xrepo_directory_scope package_name)
    if(DEFINED ${package_name}_INCLUDE_DIRS)
        message(
            STATUS
                "xrepo: directory scope include_directories(${${package_name}_INCLUDE_DIRS})"
        )
    ...
    if(DEFINED ${package_name}_LIBRARY_DIRS)
        message(
            STATUS
                "xrepo: directory scope link_directories(${${package_name}_LIBRARY_DIRS})"
        )
...
macro(_xrepo_fetch_json)
    ...
    execute_process(
        COMMAND ${XREPO_CMD} fetch ${_xrepo_deps} --json ${_xrepo_cmdargs}
    ...
    if(NOT "${exit_code}" STREQUAL "0")
        ...
        message(
            FATAL_ERROR "xrepo fetch --json failed, exit code: ${exit_code}")
    endif()
    ...
    if(NOT "${json_error}" STREQUAL "NOTFOUND")
        ...
        message(
            FATAL_ERROR
                "xrepo fetch --json: fail to parse output, error: ${json_error}"
        )
    endif()
    ...
    foreach(idx RANGE 0 ${len_end})
        ...
        elseif("${includedirs_type}" MATCHES "ARRAY|OBJECT")
            string(JSON includedirs_len ERROR_VARIABLE includedirs_error
                   LENGTH ${json_output} ${idx} "includedirs")
            if("${includedirs_error}" STREQUAL "NOTFOUND"
               AND NOT "${includedirs_len}" EQUAL 0)
        ...
        if(NOT ARG_USE_ABSOLUTE_LIBS)
            ...
            elseif("${linkdirs_type}" MATCHES "ARRAY|OBJECT")
                string(JSON linkdirs_len ERROR_VARIABLE linkdirs_error
                       LENGTH ${json_output} ${idx} "linkdirs")
                if("${linkdirs_error}" STREQUAL "NOTFOUND"
                   AND NOT "${linkdirs_len}" EQUAL 0)
        ...
        elseif("${links_type}" MATCHES "ARRAY|OBJECT")
            string(JSON links_len ERROR_VARIABLE links_error
                   LENGTH ${json_output} ${idx} ${links_tag})
            if("${links_error}" STREQUAL "NOTFOUND" AND NOT "${links_len}"
                                                        EQUAL 0)
        ...
        elseif("${syslinks_type}" MATCHES "ARRAY|OBJECT")
            string(JSON syslinks_len ERROR_VARIABLE syslinks_error
                   LENGTH ${json_output} ${idx} "syslinks")
            if("${syslinks_error}" STREQUAL "NOTFOUND"
               AND NOT "${syslinks_len}" EQUAL 0)
        ...
        elseif("${defines_type}" MATCHES "ARRAY|OBJECT")
            string(JSON defines_len ERROR_VARIABLE defines_error
                   LENGTH ${json_output} ${idx} "defines")
            if("${defines_error}" STREQUAL "NOTFOUND" AND NOT "${defines_len}"
                                                          EQUAL 0)
    ...
    if(DEFINED includedirs)
        ...
        set(${package_name}_INCLUDE_DIR
            "${includedirs}"
            CACHE INTERNAL "") # Keep compatibility, remove in the future.
        list(APPEND xrepo_vars_${package_name} ${package_name}_INCLUDE_DIRS)
        message(
            STATUS
                "xrepo: ${package_name}_INCLUDE_DIRS ${${package_name}_INCLUDE_DIRS}"
        )
    else()
        message(
            STATUS "xrepo fetch --json: ${package_name} includedirs not found")
    endif()
    ...
    if(DEFINED linkdirs)
        set(${package_name}_LIBRARY_DIRS
            "${linkdirs}"
            CACHE INTERNAL "")
        set(${package_name}_LINK_DIR
            "${includedirs}"
            CACHE INTERNAL "") # Keep compatibility, remove in the future.
        list(APPEND xrepo_vars_${package_name} ${package_name}_LIBRARY_DIRS)
        message(
            STATUS
                "xrepo: ${package_name}_LIBRARY_DIRS ${${package_name}_LIBRARY_DIRS}"
        )
    else()
    ...
    if(DEFINED links)
        set(${package_name}_LINK_LIBRARIES
            "${links}"
            CACHE INTERNAL "")
        ...
        message(
            STATUS
                "xrepo: ${package_name}_LINK_LIBRARIES ${${package_name}_LINK_LIBRARIES}"
        )
    else()
    ...
    if(DEFINED syslinks)
        set(${package_name}_SYS_LIBRARIES
            "${syslinks}"
            CACHE INTERNAL "")
        list(APPEND xrepo_vars_${package_name} ${package_name}_SYS_LIBRARIES)
        message(
            STATUS
                "xrepo: ${package_name}_SYS_LIBRARIES ${${package_name}_SYS_LIBRARIES}"
        )
    endif()

    if(DEFINED defines)
        set(${package_name}_DEFINITIONS
            "${defines}"
            CACHE INTERNAL "")
        list(APPEND xrepo_vars_${package_name} ${package_name}_DEFINITIONS)
        message(
            STATUS
                "xrepo: ${package_name}_DEFINITIONS ${${package_name}_DEFINITIONS}"
        )
    endif()

    set(_cache_xrepo_vars_${package_name}
        "${xrepo_vars_${package_name}}"
        CACHE INTERNAL "")
endmacro()
...
macro(_xrepo_fetch_cflags)
    ...
    execute_process(
        COMMAND ${XREPO_CMD} fetch --cflags ${_xrepo_cmdargs}
    ...
    if(NOT "${exit_code}" STREQUAL "0")
        ...
        message(STATUS "STDERR:\n${cflags_error_output}")
        message(
            FATAL_ERROR "xrepo fetch --cflags failed, exit code: ${exit_code}")
    endif()
    ...
    if(EXISTS "${install_dir}/lib/cmake/${package_name}")
        set(${package_name}_DIR
            "${install_dir}/lib/cmake/${package_name}"
            CACHE INTERNAL "")
...
function(xrepo_from_xmake)
    ...
    if(NOT EXISTS "${_xmakefile}")
        message(
            FATAL_ERROR "xrepo_from_xmake: xmake.lua not found at ${_xmakefile}"
        )
    endif()
    ...
    set_property(
        DIRECTORY
        APPEND
        PROPERTY CMAKE_CONFIGURE_DEPENDS "${_xmakefile}")
    ...
    set(_cache_key
        "${_xmakefile}:mtime=${_xmakefile_mtime}:platform=${XREPO_PLATFORM}:arch=${XREPO_ARCH}:toolchain=${XREPO_TOOLCHAIN}"
    )

    if("${_cache_xrepo_from_xmake_key}" STREQUAL "${_cache_key}")
        message(
            STATUS
                "xrepo_from_xmake: packages already installed (xmake.lua unchanged), skipping"
        )
        return()
    endif()
    ...
    execute_process(
        COMMAND
            ${CMAKE_COMMAND} -E env --unset=CC --unset=CXX --unset=LD
            ${XMAKE_CMD} f --yes ${_verbose} ${_platform} ${_arch} ${_toolchain}
        WORKING_DIRECTORY "${_xmakefile_dir}"
        RESULT_VARIABLE _exit_code)
    if(NOT "${_exit_code}" STREQUAL "0")
        message(
            FATAL_ERROR
                "xrepo_from_xmake: xmake f failed (exit code: ${_exit_code})")
    endif()
    ...
    execute_process(
        COMMAND
            ${CMAKE_COMMAND} -E env --unset=CC --unset=CXX --unset=LD
            ${XMAKE_CMD} require --yes ${_verbose} ${_jobs} ${_platform}
            ${_arch} ${_toolchain}
        WORKING_DIRECTORY "${_xmakefile_dir}"
        RESULT_VARIABLE _exit_code)
    if(NOT "${_exit_code}" STREQUAL "0")
        message(
            FATAL_ERROR
                "xrepo_from_xmake: xmake require failed (exit code: ${_exit_code})"
        )
    endif()

    set(XREPO_ACTIVE_XMAKEFILE
        "${_xmakefile}"
        CACHE INTERNAL "")
    set(_cache_xrepo_from_xmake_key
        "${_cache_key}"
        CACHE INTERNAL "")
endfunction()
...
function(xrepo_fetch_package package)
    ...
    set(_xrepo_cmdargs ${_platform} ${_arch} ${_toolchain} ${_includes}
                       ${_mode} ${package})

    string(REGEX REPLACE ";" " " _fetch_cmdstr
                         "${XREPO_CMD} fetch ${_deps} ${_xrepo_cmdargs}")
    if(ARG_USE_ABSOLUTE_LIBS)
        string(APPEND _fetch_cmdstr " [absolute-libs]")
    endif()

    if("${_cache_xrepo_fetch_cmdargs_${package_name}}" STREQUAL
       "${_fetch_cmdstr}")
        message(
            STATUS
                "xrepo_fetch_package: ${package_name} already fetched, using cached variables"
        )
        foreach(var ${_cache_xrepo_vars_${package_name}})
            message(STATUS "xrepo: ${var} = ${${var}}")
        endforeach()
    ...
    set(_cache_xrepo_fetch_cmdargs_${package_name}
        "${_fetch_cmdstr}"
        CACHE INTERNAL "")
endfunction()
