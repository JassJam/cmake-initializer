...
set(ENABLE_CCACHE
    ON
    CACHE BOOL "Enable ccache for faster rebuilds")
...
cmake_dependent_option(DEV_MODE "Enable development mode (all quality tools)"
                       ON "NOT RELEASE_MODE" OFF)
cmake_dependent_option(RELEASE_MODE "Enable release optimizations" OFF
                       "NOT DEV_MODE" ON)

# === GLOBAL OPTIONS ===
set(ENABLE_GLOBAL_EXCEPTIONS
    "ON"
    CACHE STRING "Enable global exception handling")
set(ENABLE_GLOBAL_WARNINGS_AS_ERRORS
    "${DEV_MODE}"
    CACHE STRING "Enable global warnings as errors")
set(ENABLE_GLOBAL_SANITIZERS
    "${DEV_MODE}"
    CACHE STRING "Enable global sanitizers")
set(ENABLE_GLOBAL_HARDENING
    "${DEV_MODE}"
    CACHE STRING "Enable global hardening")
set(ENABLE_GLOBAL_STATIC_ANALYSIS
    "${DEV_MODE}"
    CACHE STRING "Enable global static analysis")
...
option(ENABLE_ASAN "Enable address sanitizer (detects memory errors)"
       ${DEFAULT_ASAN})
...
# === DEBUG OPTIONS ===
option(ENABLE_EDIT_AND_CONTINUE "Enable Edit and Continue support (MSVC /ZI)"
       ${DEV_MODE})
option(ENABLE_DEBUG_INFO "Enable debug information generation" ${DEV_MODE})
if(NOT ENABLE_DEBUG_INFO)
    set(DEBUG_INFO_LEVEL
        "0"
        CACHE STRING
              "Debug information level (0-3 for GCC/Clang, ignored for MSVC)")
else()
    set(DEBUG_INFO_LEVEL
        "2"
        CACHE STRING
...
# === EMSCRIPTEN OPTIONS ===
option(ENABLE_EMSDK_AUTO_INSTALL
       "Automatically install EMSDK locally if not found" ON)

# === TESTING OPTIONS ==
set(BUILD_TESTING
    ON
    CACHE BOOL "Build and enable testing")
...
cmake_dependent_option(
    ...
    "Enable security hardening options (stack protection, etc.)" ON
    "ENABLE_GLOBAL_SANITIZERS OR DEV_MODE" OFF)

# ENABLE_EDIT_AND_CONTINUE is not compatible with asan, so disable it if ASan is enabled
cmake_dependent_option(
    ENABLE_EDIT_AND_CONTINUE
    "Enable Edit&Continue for debugging (requires MSVC)" ON
    "NOT ENABLE_ASAN AND NOT ENABLE_LSAN" OFF)
...
message(
    STATUS
        "Sanitizers: ${ENABLE_GLOBAL_SANITIZERS} (ASan:${ENABLE_ASAN}, UBSan:${ENABLE_UBSAN})"
)
message(STATUS "Static analysis: ${ENABLE_GLOBAL_STATIC_ANALYSIS}")
message(
    STATUS
        "Debug options: Edit&Continue:${ENABLE_EDIT_AND_CONTINUE}, DebugInfo:${ENABLE_DEBUG_INFO} (level:${DEBUG_INFO_LEVEL})"
)
